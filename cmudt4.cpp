...
{
    udt4_conn_data_ptr ucd = (udt4_conn_data_ptr) svc->malloc_func(sizeof(struct udt4_connection_data));
...
static
void
add_sock_map_data(udt4_transport_data_ptr udt, UDTSOCKET sock, udt4_conn_data_ptr ucd)
{
  if (udt->conn_count == 0) {
    udt->map = (socket_data_map_entry*)malloc(sizeof(udt->map[0]));
  } else {
    udt->map = (socket_data_map_entry*)realloc(udt->map, (udt->conn_count + 1) * sizeof(udt->map[0]));
  }
...
initiate_conn(CManager cm, CMtrans_services svc, transport_entry trans, attr_list attrs, udt4_conn_data_ptr ucd, attr_list conn_attr_list)
{
	...
	host_ip = 0;
        ...
        svc->trace_out(cm, "udt4 transport connect to host_IP %lx", host_ip);
    }
	...
	return -1;
        ...
        svc->trace_out(cm, "udt4 transport connect to port %d", int_port_num);
    }
	...
	if ((sock = UDT::socket(AF_INET, SOCK_STREAM, 0)) == SOCKET_ERROR) {
	    svc->trace_out(cm, " UDT4 connect FAILURE --> Couldn't create socket");
	    return -1;
	}
	...
	svc->trace_out(cm, "Attempting udt4 socket connection, host=\"%s\", IP = %s, port %d",
		       host_name == 0 ? "(unknown)" : host_name, ip_str,
		       ntohs(sock_addr.s_I4.sin_port));
	if (UDT::connect(sock, (struct sockaddr *) &sock_addr,
...
libcmudt4_LTX_initiate_conn(CManager cm, CMtrans_services svc, transport_entry trans, attr_list attrs)
{
    ...
    if ((sock = initiate_conn(cm, svc, trans, attrs, ucd, conn_attr_list)) < 0)
	return NULL;
...
libcmudt4_LTX_self_check(CManager cm, CMtrans_services svc, transport_entry trans, attr_list attrs)
{
    ...
    if (!query_attr(attrs, CM_IP_HOSTNAME, /* type pointer */ NULL,
    /* value pointer */ (attr_value *)(long) & host_name)) {
	svc->trace_out(cm, "CMself check udt4 transport found no IP_HOST attribute");
	host_name = NULL;
    }
    if (!query_attr(attrs, CM_IP_ADDR, /* type pointer */ NULL,
    /* value pointer */ (attr_value *)(long) & host_addr)) {
	svc->trace_out(cm, "CMself check udt4 transport found no IP_ADDR attribute");
	if (host_name == NULL) return 0;
	host_addr = 0;
    }
    if (!query_attr(attrs, CM_IP_PORT, /* type pointer */ NULL,
    /* value pointer */ (attr_value *)(long) & int_port_num)) {
	svc->trace_out(cm, "CMself check udt4 transport found no IP_PORT attribute");
	return 0;
    }
	...
	svc->trace_out(cm, "CMself check - Ports don't match, %d, %d", int_port_num, utd->listen_port);
	return 0;
    }
...
libcmudt4_LTX_connection_eq(CManager cm, CMtrans_services svc, transport_entry trans, attr_list attrs, udt4_conn_data_ptr ucd)
{
    ...
    if (!query_attr(attrs, CM_IP_PORT, /* type pointer */ NULL,
    /* value pointer */ (attr_value *)(long) & int_port_num)) {
	svc->trace_out(cm, "Conn Eq udt4 transport found no IP_PORT attribute");
	return 0;
    }
    if (!query_attr(attrs, CM_IP_ADDR, /* type pointer */ NULL,
...
libcmudt4_LTX_non_blocking_listen(CManager cm, CMtrans_services svc, transport_entry trans, attr_list listen_info)
{
    ...
    svc->trace_out(cm, "UDT4 begin listen, requested port %d", attr_port_num);
	...
	svc->trace_out(cm, "UDT4 trying to bind selected port %d", port_num);
	if (UDT::bind(conn_sock, (struct sockaddr *) &sock_addr,
    ...
    if (UDT::getsockname(conn_sock, (struct sockaddr *) &sock_addr, &length) < 0) {
	...
	if ((getenv("Cmudt4UseHostname") != NULL) || 
	    use_hostname) {
	    add_attr(ret_list, CM_IP_HOSTNAME, Attr_String,
		     (attr_value) strdup(host_name));
	} else if (IP == 0) {
...
libcmudt4_LTX_read_to_buffer_func(CMtrans_services svc, udt4_conn_data_ptr ucd, void *buffer, int requested_len, int non_blocking)
{
    ...
    svc->trace_out(ucd->utd->cm, "CMUDT4 read of %d bytes on fd %d, non_block %d", requested_len,
		   ucd->udtsock, non_blocking);
    UDT::getsockopt(ucd->udtsock, 0, UDT_RCVDATA, &rcv_size, &var_size);
    if (non_blocking && (rcv_size == 0)) {
	svc->trace_out(ucd->utd->cm, "CMUDT4 socket state is %d, rcv_size = %d, non_blocking read would block, returning 0\n", UDT::getsockstate(ucd->udtsock), rcv_size);
     return 0;
    }
...
udt4_service_epoll(void *void_trans, void *not_used)
{
	...
	if (*i == utd->listen_sock) {
	  ...
	  UDTSOCKET new_sock = UDT::accept(*i, (sockaddr*)&clientaddr, &addrlen);
	  udt4_conn_data_ptr ucd = create_udt4_conn_data(svc);
	  ucd->udtsock = new_sock;
	  ucd->utd = utd;
	  conn_attr_list = create_attr_list();
	  CMConnection conn = svc->connection_create(trans, ucd, conn_attr_list);
	  ucd->conn = conn;
...
libcmudt4_LTX_initialize(CManager cm, CMtrans_services svc, transport_entry trans)
{
    ...
    UDT::startup();
    udt4_data = (udt4_transport_data_ptr) svc->malloc_func(sizeof(struct udt4_transport_data));
    ...
    svc->add_shutdown_task(cm, free_udt4_data, (void *) udt4_data, FREE_TASK);
    udt4_data->eid = UDT::epoll_create();
    int localFD = UDT::epoll_getfd(udt4_data->eid);
    svc->trace_out(cm, "Adding FD %d to select list",
		   localFD);
...
cmudt4_add_static_transport(CManager cm, CMtrans_services svc)
{
    ...
    transport->cm = cm;
    transport->transport_init = (CMTransport_func)libcmudt4_LTX_initialize;
    transport->listen = (CMTransport_listen_func)libcmudt4_LTX_non_blocking_listen;
    transport->initiate_conn = (CMTransport_conn_func)libcmudt4_LTX_initiate_conn;
    transport->self_check = (CMTransport_self_check_func)libcmudt4_LTX_self_check;
    transport->connection_eq = (CMTransport_connection_eq_func)libcmudt4_LTX_connection_eq;
    transport->shutdown_conn = (CMTransport_shutdown_conn_func)libcmudt4_LTX_shutdown_conn;
    transport->read_block_func = (CMTransport_read_block_func)NULL;
    transport->read_to_buffer_func = (CMTransport_read_to_buffer_func)libcmudt4_LTX_read_to_buffer_func;
    transport->writev_func = (CMTransport_writev_func)libcmudt4_LTX_writev_func;
    transport->NBwritev_func = (CMTransport_writev_func) NULL;
    
    transport->set_write_notify = (CMTransport_set_write_notify_func) NULL;
    transport->get_transport_characteristics = (CMTransport_get_transport_characteristics) libcmudt4_LTX_get_transport_characteristics;
