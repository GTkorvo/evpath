...
event_handler(CManager cm, void *vevent, void *client_data, attr_list attrs)
{
	...
	if (abs(delay.tv_sec *1000000 + delay.tv_usec - 1000000) > 100000) {
	    printf("Message delayed too much, %d\n", abs(delay.tv_sec *1000000 + delay.tv_usec - 1000000));
	    failure = 1;
	}
    }
    if ((message_count % 2) == 1) {
	if (abs(delay.tv_sec *1000000 + delay.tv_usec - 1500000) > 100000) {
	    printf("Message delayed too much, %d\n", abs(delay.tv_sec *1000000 + delay.tv_usec - 1500000));
	    failure = 1;
	}
    }
    message_count++;
