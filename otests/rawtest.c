...
attr_list attrs;
{
    ...
    f = get_format_IOcontext(c, vevent);
    if (!has_conversion_IOformat(f)) {
	IOFormat *fl = get_subformats_IOformat(f);
	while (*fl != NULL) {
	    if (strcmp(name_of_IOformat(*fl), "nested") == 0) {
		set_conversion_IOcontext(c, *fl, nested_field_list,
					 sizeof(nested));
	    } else if (strcmp(name_of_IOformat(*fl), "complex") == 0) {
		set_conversion_IOcontext(c, *fl, complex_field_list,
					 sizeof(complex));
	    } else if (strcmp(name_of_IOformat(*fl), "simple") == 0) {
		set_conversion_IOcontext(c, *fl, simple_field_list,
					 sizeof(simple_rec));
	    }
	    fl++;
	}
    }
    decode_IOcontext(c, vevent, &incoming);
